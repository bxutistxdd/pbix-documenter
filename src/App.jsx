...
async function analyzeWithGroq(parsed,groqKey){
  ...
  const prompt=`Eres experto en Power BI. Analiza este modelo y genera documentación técnica interna en español.\n\nMODELO:\n${JSON.stringify(summary,null,2).substring(0,14000)}\n\nResponde SOLO con JSON válido (sin markdown):\n{"titulo":"Documentación Técnica: [nombre sin extension]","fecha":"${new Date().toLocaleDateString("es-CO",{year:"numeric",month:"long",day:"numeric"})}","resumen_ejecutivo":"3-4 oraciones.","secciones":[{"id":"fuentes_datos","titulo":"Fuentes de Datos","contenido":"...","items":[{"nombre":"...","tipo":"...","descripcion":"..."}]},{"id":"arquitectura_modelo","titulo":"Arquitectura del Modelo","contenido":"...","items":[]},{"id":"tablas_columnas","titulo":"Tablas y Columnas","contenido":"...","items":[{"tabla":"...","tipo":"Hecho/Dimensión/Staging/Parámetro","columnas_clave":["..."],"descripcion":"..."}]},{"id":"medidas_dax","titulo":"Medidas DAX","contenido":"...","items":[{"nombre":"...","tabla":"...","expresion":"...","proposito":"..."}]},{"id":"relaciones","titulo":"Relaciones del Modelo","contenido":"...","items":[{"desde":"...","hasta":"...","cardinalidad":"...","activa":true}]},{"id":"paginas_reporte","titulo":"Páginas del Reporte","contenido":"...","items":[{"pagina":"...","visuales":["..."],"proposito":"..."}]}]}`;
  const res=await fetch("https://api.groq.com/openai/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${groqKey}`},body:JSON.stringify({model:"llama-3.3-70b-versatile",max_tokens:4000,temperature:0.2,messages:[{role:"user",content:prompt}],response_format:{type:"json_object"}})});
...
function buildMarkdown(docData,parsed){
  const L=[];
  L.push(`# ${docData.titulo}`);L.push(`**Fecha:** ${docData.fecha}  `);L.push(`**Tablas:** ${parsed.tables.length} | **Medidas:** ${parsed.measures.length} | **Relaciones:** ${parsed.relationships.length} | **Páginas:** ${parsed.pages.length}`);L.push("");L.push("---");L.push("");
  L.push("## Resumen Ejecutivo");L.push(docData.resumen_ejecutivo);L.push("");
  for(const sec of docData.secciones){
    L.push(`## ${sec.titulo}`);if(sec.contenido){L.push(sec.contenido);L.push("");}
...
export default function App(){
  ...
  const run=async()=>{
    ...
    try{for(const file of files){
      addLog(`Procesando: ${file.name}`,"section");setProgress(5);addLog("Extrayendo estructura...","info");
      const parsed=await parseFile(file);setProgress(35);
      ...
      const docData=await analyzeWithGroq(parsed,groqKey.trim());setProgress(85);addLog("✓ Análisis completado","ok");
      const md=buildMarkdown(docData,parsed);
      const url=URL.createObjectURL(new Blob([md],{type:"text/markdown"}));setProgress(100);addLog("✅ Listo","ok");
      setResult({url,name:file.name.replace(/\.(pbix|pbit)$/,"_documentacion.md"),docData,parsed});
    }}catch(e){addLog(`Error: ${e.message}`,"error");}finally{setRunning(false);}
  };
  return(
    <div style={{background:C.dark,minHeight:"100vh",fontFamily:"'Segoe UI',system-ui,sans-serif",color:C.text}}>
      <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:"14px 24px",display:"flex",alignItems:"center",gap:12}}>
        ...
        <div><div style={{fontWeight:700,fontSize:15,color:C.yellow}}>PBIX / PBIT Documenter</div><div style={{fontSize:11,color:C.muted}}>Groq · llama-3.3-70b-versatile · Gratis</div></div>
      </div>
      <div style={{maxWidth:760,margin:"0 auto",padding:"20px"}}>
        ...
        {files.length>0&&(<div style={{marginTop:14}}>
          ...
          <button onClick={run} disabled={running} style={{width:"100%",marginTop:4,padding:13,background:running?"#5a4a10":"linear-gradient(135deg,#f7c948,#f59e0b)",color:"#0f1117",fontWeight:700,fontSize:14,border:"none",borderRadius:10,cursor:running?"not-allowed":"pointer"}}>{running?"⏳ Procesando...":"⚡ Generar Documentación"}</button>
        ...
        {result&&(<div style={{background:"#0d1f0d",border:"1px solid #276227",borderRadius:10,padding:20,marginTop:16}}>
          ...
          <div style={{background:"#0a0d13",border:`1px solid ${C.border}`,borderRadius:8,padding:14,marginBottom:14,maxHeight:180,overflowY:"auto",fontSize:12,color:C.muted,whiteSpace:"pre-wrap",lineHeight:1.6}}>{result.docData.resumen_ejecutivo}{"\n\n"}{result.docData.secciones?.map(s=>`## ${s.titulo}\n${s.contenido||""}`).join("\n\n").substring(0,600)}...</div>
          <a href={result.url} download={result.name} style={{display:"inline-block",padding:"10px 22px",background:"#276227",color:C.green,border:"1px solid #276227",borderRadius:8,fontWeight:600,fontSize:13,textDecoration:"none"}}>⬇ Descargar {result.name}</a>
        </div>)}
      </div>
    </div>
  );
